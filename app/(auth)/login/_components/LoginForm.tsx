...
export default function LoginForm() {
  ...
  const onVerifyToken = async (data: VerifyOutput) => {
    ...
    try {
      ...
      setLoggedIn({
        ...
        loginData: {
          token: "mock-token",
          meus_dados: { id: "1", email: selectedChannel === "email" ? contactValue : "mock@exemplo.com", nome: "Mock" },
        },
      });
      ...
      const message = error instanceof Error ? error.message : "Erro inesperado ao validar token.";
      setFeedbackMessage(message);
  ...
  const selectLoginMethod = (method: "email" | "whatsapp") => {
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          disabled={isLoading}
          className={`flex items-center justify-center gap-2 rounded-md border border-custom-light-400 px-3 py-2 text-sm font-montserrat transition-colors disabled:opacity-50 ${
            loginMethod === "email" ? "bg-white text-black" : "bg-white/80 text-black hover:bg-white"
          }`}
        >
        ...
        <button
          ...
          disabled={isLoading}
          className={`flex items-center justify-center gap-2 rounded-md border border-custom-light-400 px-3 py-2 text-sm font-montserrat transition-colors disabled:opacity-50 ${
            loginMethod === "whatsapp" ? "bg-white text-black" : "bg-white/80 text-black hover:bg-white"
          }`}
        >
      ...
      {step === "send" ? (
        <form onSubmit={handleSubmitSend(onSendToken)} className="space-y-5">
          <div>
            <label htmlFor="value" className="block text-white font-montserrat font-medium text-sm mb-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(event) => {
                  const nextValue =
                    selectedChannel === "whatsapp" ? formatWhatsapp(event.target.value) : event.target.value;
                ...
                className={`w-full pl-10 pr-4 py-3 border ${
                  sendErrors.value ? "border-red-500 focus:ring-red-500" : "border-custom-light-400 focus:ring-tints-french-blue"
                } rounded-md font-montserrat text-sm bg-white focus:outline-none focus:ring-2 transition-all`}
                placeholder={selectedChannel === "email" ? "seu@email.com" : "(62) 99999-9999"}
                autoComplete={selectedChannel === "email" ? "email" : "tel"}
              />
            </div>
            {sendErrors.value && (
              <p className="mt-1 text-red-500 font-montserrat text-xs">{sendErrors.value.message}</p>
            )}
          </div>
        ...
        <form onSubmit={handleSubmitVerify(onVerifyToken)} className="space-y-5">
          ...
          <div>
            <label htmlFor="token" className="block text-white font-montserrat font-medium text-sm mb-2">
              Token de validação
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 border ${
                  verifyErrors.token ? "border-red-500 focus:ring-red-500" : "border-custom-light-400 focus:ring-tints-french-blue"
                } rounded-md font-montserrat text-sm bg-white focus:outline-none focus:ring-2 transition-all`}
            ...
            {verifyErrors.token && (
              <p className="mt-1 text-red-500 font-montserrat text-xs">{verifyErrors.token.message}</p>
            )}
          </div>
