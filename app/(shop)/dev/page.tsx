...
export default function DevPage() {
  ...
  const actions = useMemo(
    () => [
      { label: "TokenService: gerar token", url: "/api/dev/tokenservice/generate", init: { method: "POST" as const } },
      { label: "TokenService RAW: gerar token", url: "/api/dev/tokenservice/raw/generate", init: { method: "POST" as const } },
      {
          ...
          <div className="flex flex-wrap gap-2">
            {actions.map((action) => (
              <button
                ...
                disabled={Boolean(loading)}
                className="h-10 px-3 rounded-md bg-tints-french-blue text-white font-montserrat text-sm font-semibold disabled:opacity-60"
              >
                {loading === action.url ? "Carregando..." : action.label}
              </button>
            ))}
          </div>
        </div>
