...
export default function CategoriaPage({ params }: { params: Promise<{ slug: string[] | string }> }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const items = Array.isArray(result.data) ? result.data.map(toProductItem).filter(Boolean) : [];

        if (!active) return;

        setCategoria(found);
        setProducts(items as ProductItem[]);
        setTotal(asNumber(result.total, (items as ProductItem[]).length));
        setTotalPages(asNumber(result.totalPages, 0));
      } catch (error) {
        if (!active) return;

        const message =
          error instanceof Error && error.message === "categoria_nao_encontrada"
            ? "Categoria não encontrada."
            : "Não foi possível carregar a categoria agora.";
  ...
  }, [loadCategoriaBySlug, loadProdutosByCategoria, page, slugPath]);
        ...
        <div className="max-w-7xl mx-auto rounded-xl border border-custom-light-300 bg-white p-4 md:p-5 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-end gap-3 md:gap-4">
            <div className="w-full lg:max-w-sm">
              ...
              <select
                value={normalizeSlugPath(categoria?.slug) ?? ""}
                onChange={(e) => {
