...
import { createPix } from "@/lib/integration/checkoutService";
...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json().catch(() => null)) as { ttlMinutos?: unknown } | null;
    const ttlMinutos = body?.ttlMinutos == null ? undefined : Number.parseInt(String(body.ttlMinutos), 10);

    if (!Number.isFinite(parsedCheckoutId)) {
      return NextResponse.json({ success: false, message: "checkoutId invalido." }, { status: 400 });
    }

    const result = await createPix(parsedCheckoutId, Number.isFinite(ttlMinutos) ? { ttlMinutos } : {});
