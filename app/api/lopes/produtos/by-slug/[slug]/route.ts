...
const DEFAULT_FLAGS: OverwriteFlags = { category: 0, brand: 1, image: 0, name: 0, slug: 0 }
...
async function tryApplyOverwriteFromRedis(input: { produto: ProdutoMock; idProduto: number }): Promise<ProdutoMock> {
  try {
    ...
    const flagsDoc = await tryReadRedisJson(`${prefix}:product_overwrite:global`)
...
export async function GET(_request: Request, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    await tryApplyOverwriteFromRedis({ produto, idProduto })
