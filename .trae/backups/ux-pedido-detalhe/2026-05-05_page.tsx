...
function buildResumo(pedido: PedidoDetalheUI): Array<{ label: string; value: string }> {
  ...
  const payload = parseJsonObject(rawPayload) ?? (asRecord(rawPayload) ?? {});

  const total = safeNumber(payload.valor, pedido.total);
  const desconto = safeNumber(payload.valorDesconto, 0);
  const frete = safeNumber(payload.valorFrete, 0);
  const taxas = safeNumber(payload.valorTaxas, 0);

  const subtotal = Math.max(0, total + Math.max(0, desconto));
  return [
    ...
    { label: "Taxas", value: taxas > 0 ? formatCurrency(taxas) : "—" },
  ];
}
      ...
      <section className="rounded-2xl border border-custom-light-300 bg-white p-4 shadow-sm sm:p-5">
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-montserrat text-custom-light-600">
              <span>Data: {selectedPedido ? formatDateTime(selectedPedido.createdAt) : "—"}</span>
              <span>•</span>
              <span>
                Pag.:{" "}
                <span className="font-semibold text-custom-dark-1000">
                  {selectedPedido ? selectedPedido.pagamentoMetodo : "—"}
                </span>
              </span>
            </div>
          ...
          <div className="space-y-4 lg:order-2">
            ...
            <Card className="border-custom-light-300 shadow-sm">
              ...
              <CardContent className="space-y-3 px-4 pb-5 sm:px-5">
                ...
                {normalizeStatusLabel(selectedPedido.pagamentoStatus) !== normalizeStatusLabel(selectedPedido.status) && (
                  <div className="flex items-center justify-between">
                    <div className="text-xs font-montserrat text-custom-light-600">Status</div>
                    <div className="text-sm font-montserrat font-semibold text-custom-dark-1000">
                      {normalizeStatusLabel(selectedPedido.pagamentoStatus)}
                    </div>
                  </div>
                )}
            ...
            <Card className="border-custom-light-300 shadow-sm">
              ...
              <CardContent className="space-y-3 px-4 pb-5 sm:px-5">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm font-montserrat text-custom-dark-1000">{selectedPedido.freteNome || "—"}</div>
                  <div className="text-xs font-montserrat text-custom-light-600">
